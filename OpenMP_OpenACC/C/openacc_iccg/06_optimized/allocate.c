...
void** allocate_matrix(int size,int m,int n)
{
  ...
  for(i=1;i<m;i++) aa[i]=(void*)aa[i-1]+size*n;
  return aa;
}

void allocate_matrix2(void ***aa, int size,int m,int n)
{
  int i;
  if ( ( aa[0]=(void ** )malloc( m * sizeof(double*) ) ) == NULL ) {
     fprintf(stdout,"Error:Memory does not enough! aa in matrix \n");
     exit(1);
  }
  if ( ( aa[0][0]=(void * )malloc( m * n * size ) ) == NULL ) {
     fprintf(stdout,"Error:Memory does not enough! in matrix \n");
     exit(1);
  }
  for(i=1;i<m;i++) aa[0][i]=(void*)aa[0][i-1]+size*n;
  //  return aa;
}
/**************************
  deallocate matrix 
